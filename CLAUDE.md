...
**Next sprint entry point:** Sprint 19 (Track B Sprint 6) — **not yet scoped.** Two paired items, both needing a live podman stack so they share a sprint: **(1) runtime-clear Sprint 18 B-6** — `podman compose up -d --build` shows kafka/auth healthy before order/inventory + the containerized Playwright smoke confirms the reservation feed grows by exactly one (the live proof Codex couldn't run); **(2) harden `scripts/startup-all.sh`'s bare-podman fallback** (Dimitri approved "when it fits") — replace sleep-based sequencing with `podman run --health-cmd` / health-polls so the no-compose-provider path gets the same readiness gating the compose path now has. Run dependency-currency check at scoping. **Deferred future backlog (unchanged):** Vercel frontend deploy prep (vercel.json rewrites / env API base URL + CORS on the 3 Spring services); real Google OAuth end-to-end + CI (needs a "no real secrets" policy decision first). 
