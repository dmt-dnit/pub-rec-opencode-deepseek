...
**Next sprint entry point:** Sprint 18 (Track B Sprint 5) — **scoping now (2026-07-01).** Sprint 18 backlog: **B-6** (Docker Compose, its own sprint), outbox relay `FOR UPDATE SKIP LOCKED` nicety, Angular-22.x dev-CVE patch watch. Watch-item: bump Angular off 22.0.4 when a patched 22.x ships (clears the dev-only http-proxy-middleware CVE). Run dependency-currency check at scoping (Boot 4.1.0 / Spring Security 7.1.0 / Angular 22.0.4 / Java 21 all current as of 2026-06-29). 
