...
**Next sprint entry point:** Sprint 17 (Track B Sprint 4) — not yet scoped. Backlog: inventory exactly-once/outbox hardening (Sprint 14 follow-up 2), **B-2** (observability), **B-3** (Testcontainers + contract tests), **B-6** (Docker Compose, its own sprint). Watch-item: bump Angular off 22.0.4 when a patched 22.x ships (clears the dev-only http-proxy-middleware CVE). Run dependency-currency check at scoping (Boot 4.1.0 / Spring Security 7.1.0 / Angular 22.0.4 / Java 21 all current as of 2026-06-29). 
