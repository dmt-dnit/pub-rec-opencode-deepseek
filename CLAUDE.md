...
**Next sprint entry point:** Sprint 18 (Track B Sprint 5) — not yet scoped. **Sprint 17 implemented & coordinator-verified, awaiting Codex** (`docs/backlog/sprint-17-handoff.md`; commits `49c74ca`→`54e0318`): B-2 (correlation IDs + ECS structured logs + actuator), B-3 (Testcontainers real-Kafka + event contract test), B-O1 (inventory transactional outbox = exactly-once). All 4 modules `mvnw verify` green (my own run); cherry-pick/FF integration preserved all Sprint 16 state. Testing the reviewer auto-pickup script this sprint. Sprint 18 backlog: **B-6** (Docker Compose, its own sprint), outbox relay `FOR UPDATE SKIP LOCKED` nicety, Angular-22.x dev-CVE patch watch. Watch-item: bump Angular off 22.0.4 when a patched 22.x ships (clears the dev-only http-proxy-middleware CVE). Run dependency-currency check at scoping (Boot 4.1.0 / Spring Security 7.1.0 / Angular 22.0.4 / Java 21 all current as of 2026-06-29). 
