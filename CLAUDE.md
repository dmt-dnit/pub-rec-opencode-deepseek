...
**Last cleared:** Sprint 17 (Track B Sprint 4) — cleared 2026-07-01. Codex round-2 confirmed all 3 fixes correct **at source** (B-2 correlation IDs + ECS logs + actuator, B-3 Testcontainers real-Kafka + event contract test, B-O1 inventory transactional outbox = exactly-once); its round-2 REJECT was purely **stale local `target/` artifacts** (git-ignored, never committed) — cleared in round 3: CLAUDE.md corrected (`b43cb71`), working tree made pristine, round-3 handoff (`13b3a0f`). Codex declined a fresh review as no-material-change → the reject reason is gone. One live-only carryover: **F2 containerized smoke** (inventory-UI feed shows no duplicate) — folds into B-6. Commits `49c74ca`→`54e0318` (+ round-2 `76b77cc`/`68c8f77`/`ae1b3c0`). Reviewer auto-pickup loop exercised across all 3 rounds — lesson banked: the loop's reviewer step must clean-build first, else stale `target/` causes false rejects ([[feedback-review-build-from-source]]).
**Next sprint entry point:** Sprint 18 (Track B Sprint 5) — **scoped (2026-07-01); briefs written, ready to implement.** Single task: **B-6** (one-command Docker Compose full stack: healthchecks + `condition: service_healthy` ordering, `scripts/startup-all.sh`) **+ close Sprint 17 F2** (add the missing `e2e/smoke.spec.ts` assertion that the inventory-UI reservation feed grows by exactly baseline+1 — no duplicate). Brief: `docs/backlog/tasks/sprint-18/B-6-docker-compose-full-stack.md`. Scoping decisions: **UIs stay on `npm start`, NOT containerized** — frontends will deploy to **Vercel**, so a UI container is throwaway (compose stays backend-only). Outbox **`FOR UPDATE SKIP LOCKED` DROPPED** — no-op on single-node H2; Sprint 17 `@Lock(PESSIMISTIC_WRITE)` already closed the race. Dependency currency (2026-07-01): all current; Angular latest **stable still 22.0.4** (no patched 22.x — `22.1.0-next.3` is pre-release), dev-CVE watch continues, not a task. **Deferred future backlog:** Vercel frontend deploy prep (vercel.json rewrites / env API base URL + CORS on the 3 Spring services); real Google OAuth end-to-end + CI (needs a "no real secrets" policy decision first). 
