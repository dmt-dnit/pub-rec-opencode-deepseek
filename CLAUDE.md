...
**Next sprint entry point:** Sprint 17 (Track B Sprint 4) — **scoped & briefed, implementing.** B-2 (observability: correlation IDs + structured JSON logs + actuator), B-3 (Testcontainers + event contract test), B-O1 (inventory transactional outbox = exactly-once). Overview `docs/backlog/sprint-17.md`; briefs in `docs/backlog/tasks/sprint-17/`. B-2+B-3 parallel (Claude agents) then B-O1 (DeepSeek) on settled main. Reviewer auto-pickup script being tested this sprint. Deferred to Sprint 18: **B-6** (Docker Compose, its own sprint). Watch-item: bump Angular off 22.0.4 when a patched 22.x ships (clears the dev-only http-proxy-middleware CVE). Run dependency-currency check at scoping (Boot 4.1.0 / Spring Security 7.1.0 / Angular 22.0.4 / Java 21 all current as of 2026-06-29). 
