...
**Last cleared:** Sprint 18 (Track B Sprint 5) — **code-reviewed at source, no blockers, 2026-07-01** (`reviews/sprint-18-track-b-review.md`). Single task **B-6**: `docker-compose.yml` healthchecks (kafka `kafka-broker-api-versions`; auth/order/inventory `curl /actuator/health`, curl added to the 3 JRE Dockerfiles) + order/inventory `depends_on … condition: service_healthy` on kafka+auth; **F2 closed** in `e2e/smoke.spec.ts` (baseline-relative `toHaveCount(initialFeedCount+1)`, dirty-DB safe). Codex verified all wiring against real templates and **endorsed the coordinator's drop of the implementer's risky zookeeper `ruok` healthcheck** (would've deadlocked startup; kafka's own healthcheck still gates the app services). Commits `03c963d` (impl) + handoff. **Stale-base trap avoided again** — the sonnet worktree agent branched pre-Sprint-18-docs; only its 5 code files were applied onto main, not the whole branch (would've reverted the snapshot + deleted the sprint docs) ([[feedback-claude-worktree-stale-base]]). **Open = runtime clearing only** (no code defect): live `podman compose up` → all healthy in order, + containerized Playwright smoke showing feed +1. Not runnable in coordinator or Codex sessions (no compose provider / live socket / browser) — folds into Sprint 19. (Sprint 17 cleared 2026-07-01: B-2 observability, B-3 Testcontainers+contract, B-O1 outbox exactly-once; its F2 carryover was closed here.)
**Next sprint entry point:** Sprint 19 (Track B Sprint 6) — **scoped (2026-07-01); briefs written, ready to implement.** Overview `docs/backlog/sprint-19.md`. Two loosely-coupled tasks: **B-9** (headline) — CI-automate the containerized E2E smoke: a new `ci.yml` job does `docker compose up -d --build`, waits for healthy, starts both UIs on `npm start`, runs the Playwright smoke incl. the F2 `toHaveCount(+1)` assertion → **this is how B-6's runtime item gets cleared, permanently and automatically** (Dimitri chose CI-automation over a one-off manual pass, 2026-07-01); **B-8** — harden `scripts/startup-all.sh`'s bare-podman fallback (replace fixed `sleep`s with bounded health-polls: kafka `broker-api-versions`, services `/actuator/health`, auth JWKS). Briefs: `docs/backlog/tasks/sprint-19/{B-9-ci-containerized-e2e-smoke,B-8-harden-podman-fallback}.md`. Both = Claude sonnet worktree agents, parallelizable. **Verification reality:** B-9's pass signal is a **live CI run** (coordinator can only `actionlint`; pushing workflow files needs `workflow` OAuth scope); B-8's live path builds 3 images (podman-box or Codex verifies; `shellcheck` otherwise). Dependency currency (2026-07-01): all current; Angular stable still 22.0.4 (watch continues). **Deferred future backlog (unchanged):** Vercel frontend deploy prep (vercel.json rewrites / env API base URL + CORS on the 3 Spring services); real Google OAuth end-to-end + CI (needs a "no real secrets" policy decision first). 
