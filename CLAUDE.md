...
**Last cleared:** Sprint 19 (Track B Sprint 6) — **delivered 2026-07-01; live CI all green including the new `e2e-smoke` job on first real run.** That green run **automatically clears Sprint 18 B-6's only open item** (the containerized runtime proof that no session — Codex, coordinator, worktree agent — could run): the stack comes up healthy in order and the Playwright smoke passes incl. the F2 `toHaveCount(+1)`. Implementer **opencode+DeepSeek** for both. **B-9** (`e305061`) — new isolated `ci.yml` `e2e-smoke` job: `docker compose up -d --build` → poll `docker inspect` health (180s) → start both UIs on `npm start` (wait :4200/:4201) → `npx playwright install --with-deps chromium && playwright test` → upload report/traces/logs on failure → `down -v`. **B-8** (`8d2bbd8`) — `startup-all.sh` bare-podman fallback: `wait_for` helper (bounded, `exit 1` on timeout) replaces the fixed `sleep`s (ZK TCP probe, kafka `broker-api-versions`, order/inventory `/actuator/health`); exec bit `100755`; shellcheck clean. Handoff `docs/backlog/sprint-19-handoff.md`; `pre-review-check.sh 19` passed. **Codex auto-pickup review pending** (CI green is the runtime proof; Codex still gives the independent source verdict). Process note: first opencode dispatch tangled (pre-made worktrees + concurrent runs) — recovered with no repo damage, re-ran B-8 solo ([[feedback-opencode-worktree-sandbox]]). (Sprint 18 B-6 cleared at source 2026-07-01, `reviews/sprint-18-track-b-review.md`; Codex endorsed dropping the risky ZK `ruok` healthcheck. Sprint 17 cleared 2026-07-01.)
**Next sprint entry point:** Sprint 20 (Track B Sprint 7) — **not yet scoped.** Track B hardening backlog is now largely drained (observability, Testcontainers, outbox exactly-once, OpenAPI, SCA, Docker Compose + healthchecks, CI containerized E2E smoke, fallback hardening all done). Run dependency-currency at scoping. **Candidate backlog / deferred (unchanged):** Vercel frontend deploy prep (vercel.json rewrites / env API base URL + CORS on the 3 Spring services); real Google OAuth end-to-end + CI (needs a "no real secrets" policy decision first); promote the Snyk CI gate from report-only (`continue-on-error`) to blocking once the dev-only Angular CVEs clear; bump Angular off 22.0.4 when a patched 22.x ships. Likely time to consider whether Track B is complete and what Track C is. 
